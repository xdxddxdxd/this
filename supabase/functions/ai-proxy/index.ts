...
// Set CEREBRAS_API_KEY, GROQ_API_KEY, OPENROUTER_API_KEY and GEMINI_API_KEY as Supabase secrets.
...
serve(async (request) => {
  ...
  try {
    ...
    const key = CHAT_ENDPOINTS[provider] ? Deno.env.get(`${provider.toUpperCase()}_API_KEY`) : provider === 'gemini' ? Deno.env.get('GEMINI_API_KEY') : Deno.env.get('GROQ_API_KEY');
    ...
    const endpoint = CHAT_ENDPOINTS[provider] || CHAT_ENDPOINTS.groq;
    const resolvedModel = provider === 'cerebras' ? (model || 'zai-glm-4.7') : model;
    const response = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: resolvedModel, response_format: { type: 'json_object' }, temperature: 0, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: prompt }] }) });
    const payload = await response.json();
